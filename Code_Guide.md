...
Validation for these changes is limited to source inspection and the authorized four Java/client compilation tasks. The authorized command passed all four tasks using the installed JDK 26 with the configured Java 21/25 release levels. The default Java 8 could not run Gradle, and the wrapper needed approval to write its existing cache lock outside the workspace sandbox. In-game pose, camera, and imported-asset behavior were not verified in this workspace. Neither target was unavailable.
