...
public final class BackendInteractor {
    ...
    public static void initializeClient() {
        ...
        ClientPlayNetworking.registerGlobalReceiver(ModelPackets.PlayerShapeSettingsPayload.ID, (payload, context) ->
                context.client().execute(() -> SkinManager.acceptServerShapeSettings(payload.uuid(), payload.settings())));
        ClientPlayNetworking.registerGlobalReceiver(ModelPackets.PlayerVoiceLevelPayload.ID, (payload, context) ->
    ...
    public static void flushShapeSettings() {
        ...
        if (pendingShapeSettings == null || now - lastShapeSync < SHAPE_SYNC_INTERVAL_NANOS
                || !canSend(ModelPackets.UpdateShapeSettingsPayload.ID)) return;
    ...
    public static void sendVoiceLevel(float level) {
        if (!canSend(ModelPackets.VoiceLevelPayload.ID)) return;
