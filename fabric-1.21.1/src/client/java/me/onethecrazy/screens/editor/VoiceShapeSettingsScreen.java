...
public class VoiceShapeSettingsScreen extends Screen {
    ...
    public VoiceShapeSettingsScreen(Screen parent, CacheSkin selectedCache) {
        super(Text.translatable("gui.fbxplayermodels.voice_shape_title"));
        this.parent = parent;
    ...
    protected void init() {
        ...
        addControl(ButtonWidget.builder(label("target", choice("target", settings.target().name())), button -> {
            settings.target = VoiceShapeSettings.Target.values()[(settings.target().ordinal() + 1) % 3];
            settings.name = "";
            settings.shapeKeyId = "";
            saveSettings();
            clearAndInit();
        }).dimensions(controlsX, 0, controlsWidth, 20).build());
        ButtonWidget name = ButtonWidget.builder(label("name", selectedName()), button -> openTargetSelection())
                .dimensions(controlsX, 0, controlsWidth, 20).build();
        name.active = settings.target() != VoiceShapeSettings.Target.NONE && !targets().isEmpty();
        name.setTooltip(Tooltip.of(Text.of(selectedName())));
        addControl(name);
        addControl(ButtonWidget.builder(label("input", choice("input", settings.input().name())), button -> {
        ...
        addControl(ButtonWidget.builder(label("response", choice("response", settings.response().name())), button -> {
        ...
        addControl(ButtonWidget.builder(label("preview", choice("preview", livePreview ? "LIVE" : "MANUAL")), button -> {
        ...
        addDrawableChild(ButtonWidget.builder(Text.translatable("gui.fbxplayermodels.shape_keys_reset_view"), button -> {
            yaw = pitch = 0f;
            zoom = 1f;
        }).dimensions(MARGIN, footerY, buttonWidth, 20).build());
        addDrawableChild(ButtonWidget.builder(Text.translatable("gui.fbxplayermodels.voice_shape_test"), button -> {
            pulseUntil = System.nanoTime() + 1_000_000_000L;
        }).dimensions(MARGIN + buttonWidth + 6, footerY, buttonWidth, 20).build());
        addDrawableChild(ButtonWidget.builder(Text.translatable("gui.fbxplayermodels.shape_keys_back"), button -> close())
                .dimensions(MARGIN + (buttonWidth + 6) * 2, footerY, buttonWidth, 20).build());
        configureInput();
    }

    private Text label(String key, Object value) {
        return Text.translatable("gui.fbxplayermodels.voice_shape_" + key, value);
    }

    private String choice(String key, String value) {
        return Text.translatable("gui.fbxplayermodels.voice_shape_" + key + "_" + value.toLowerCase(java.util.Locale.ROOT)).getString();
    }
    ...
    private void openTargetSelection() {
        MinecraftClient.getInstance().setScreen(new VoiceTargetSelectionScreen(this, targets(), entry -> {
    ...
    private void renderContent(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        context.drawText(textRenderer, textRenderer.trimToWidth(modelName == null ? "" : modelName, this.width - MARGIN * 2),
        ...
        context.drawText(textRenderer, textRenderer.trimToWidth(status, this.width - MARGIN * 2), MARGIN, 43, 0xFFAAAAAA, false);
        ...
        String hint = Text.translatable("gui.fbxplayermodels.shape_keys_preview_hint").getString();
        context.drawText(textRenderer, textRenderer.trimToWidth(hint, this.width >= 480 ? previewSize : this.width - MARGIN * 2),
        ...
        context.drawText(textRenderer, textRenderer.trimToWidth(Text.translatable("gui.fbxplayermodels.voice_shape_settings").getString(), controlsWidth),
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        ...
        if (button == 0 && maxScrollOffset() > 0 && mouseX >= scrollbarX() && mouseX < scrollbarX() + SCROLLBAR_WIDTH
    ...
    public boolean mouseReleased(double mouseX, double mouseY, int button) {
        if (button == 0 && (rotating || draggingScrollbar)) {
            rotating = false;
    ...
    private record Control(ClickableWidget widget, int row) {}
    ...
    private final class SettingSlider extends SliderWidget {
        ...
        private SettingSlider(String key, String unit, double min, double max, double step,
            ...
            super(controlsX, 0, controlsWidth, 20, Text.empty(),
        ...
        protected void applyValue() {
            ...
            if (previewOnly) {
                livePreview = false;
                for (Control control : controls) {
                    if (control.widget() instanceof ButtonWidget button && control.row() == rowCount - 2)
