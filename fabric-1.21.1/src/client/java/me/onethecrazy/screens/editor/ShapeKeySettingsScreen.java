...
public class ShapeKeySettingsScreen extends Screen {
    ...
    public ShapeKeySettingsScreen(Screen parent, CacheSkin selectedCache) {
        super(Text.translatable("gui.fbxplayermodels.shape_keys_title"));
        this.parent = parent;
    ...
    protected void init() {
        ...
        for (int i = 0; i < shapeKeys.size(); i++) {
            ...
            slider.setTooltip(Tooltip.of(Text.of(key.name())));
            sliders.add(slider);
        ...
        ButtonWidget reset = ButtonWidget.builder(Text.translatable("gui.fbxplayermodels.shape_keys_reset"), button -> {
    ...
    private void renderContent(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        String profile = Text.translatable("gui.fbxplayermodels.shape_keys_profile_status", shapeKeys.size()).getString();
        context.drawText(textRenderer, textRenderer.trimToWidth(profile, controlsWidth), controlsX, listTop - 18, 0xFFFFFFFF, true);
        ...
        if (shapeKeys.isEmpty()) {
            ...
            for (String key : selectedCache == null
                ...
                for (var line : textRenderer.wrapLines(Text.translatable(key), controlsWidth)) {
                    context.drawText(textRenderer, line, controlsX, textY, 0xFFCCCCCC, false);
            ...
            for (int i = 0; i < shapeKeys.size(); i++) {
                ...
                if (y + textRenderer.fontHeight < listTop || y > listBottom) continue;
                context.drawText(textRenderer, textRenderer.trimToWidth(shapeKeys.get(i).name(), controlsWidth), controlsX, y, 0xFFFFFFFF, true);
            }
        }
    ...
    private final class ShapeKeySlider extends SliderWidget {
        ...
        protected void updateMessage() {
            setMessage(Text.translatable("gui.fbxplayermodels.shape_keys_weight", Math.round(value * 100)));
        }
